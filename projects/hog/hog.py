...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for _ in range(num_rolls):
        current = dice()
        sow_sad = True if current == 1 else sow_sad
        total += current
...
def boar_brawl(player_score, opponent_score):
    ...
    return max(1, abs(ones_digit - tens_digit) * 3)
    # END PROBLEM 2
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for player_score in range(goal):
        for opponent_score in range(goal):
            current = strategy(player_score, opponent_score)
            if current != first:
                return False
    return True
    # END PROBLEM 7
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    return 0 if abs(sus_update(0, score, opponent_score) - score) >= threshold else num_rolls
    # END PROBLEM 11


def final_strategy(score, opponent_score):
    """This strategy is the sus strategy with some optimizations.
    It returns 0 dice when it will ensure a win and
    it will returns fewer dice when necessary to do so.
    """
    # BEGIN PROBLEM 12
    zero_roll_score = sus_update(0, score, opponent_score)
    if zero_roll_score >= GOAL:
        return 0
